...
import { isSecurityDenied } from "./security-path.js";
...
export async function walkTree(root, opts = {}) {
  ...
  async function visit(dirAbs, relDir) {
    ...
    for (const ent of entries) {
      ...
      // Do not follow symlinks out of the project (secret packing risk)
      if (lst.isSymbolicLink()) {
        skipped++;
        continue;
      }

      if (pathMatchesIgnore(rel, lst.isDirectory(), rules)) {
        skipped++;
        continue;
      }
      // Hard security denylist cannot be undone by !.env in project gitignore
      if (isSecurityDenied(rel, lst.isDirectory())) {
        skipped++;
        continue;
      }

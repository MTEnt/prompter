...
function which(bin) {
  ...
  const r = spawnSync(cmd, [bin], { encoding: "utf8", shell: process.platform === "win32" });
...
/**
 * Write prompt to a private temp file (avoids shell length limits / quoting hell).
 */
...
/** Env passed to agents: strip Prompter API keys. */
export function agentSafeEnv(extra = {}) {
  const env = { ...process.env, ...extra };
  for (const k of Object.keys(env)) {
    if (
      /^(OPENAI|XAI|ANTHROPIC|OPENROUTER|GEMINI|GOOGLE)_API_KEY$/i.test(k) ||
      /^(PROMPTER_LLM_API_KEY)$/i.test(k) ||
      k === "PROMPTER_TOKEN"
    ) {
      delete env[k];
    }
...
export function buildAgentInvocation(agentId, prompt, opts = {}) {
  ...
  // Prefer prompt file for long briefs (all agents) to avoid ARG_MAX / quoting issues
  let args = built.args;
  let promptFile = null;
  if (
    (pick.agent.id === "grok" || pick.agent.id === "agent") &&
    (runOpts.headless || prompt.length > 1200)
  ) {

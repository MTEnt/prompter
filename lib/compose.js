...
export function composePrompt(opts) {
  ...
  // Project-aware grounding (folder was attached in the UI).
  if (projectContext) {
    scaffold = `${scaffold}
...
## User request (rewrite this into a sharp agent brief)
${input}

Working directory for the agent: ${projectPath || "(attached project)"}
Prefer citing real paths from the excerpts above.`;
  }

  const extraParts = [opts.extraContext || ""];
  if (projectContext) {
    extraParts.push(
      "A real local project is attached. The improved prompt must name real files from that project when relevant."
    );
  }
  ...
  const improved = improvePromptLocal({
    ...
    preserveStructure: Boolean(direction.templateId) || Boolean(projectContext),
  });

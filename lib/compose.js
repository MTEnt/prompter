...
import { getTemplate, renderTemplate, exportForTool } from "./library.js";
 ...
 * @param {string} [opts.projectContext] - ranked project excerpts + tree
 * @param {string[]} [opts.projectFiles] - paths used in projectContext
...
export function composePrompt(opts) {
  ...
  const patterns = direction.patterns || [];
  const projectContext = String(opts.projectContext || "").trim();
  const projectPath = opts.projectPath || "";
  const projectFiles = opts.projectFiles || [];
  ...
  // Pattern ids are tracked in meta only; templates already encode habits.

  // Project-aware grounding once (do not re-append user text).
  if (projectContext) {
    scaffold = `${scaffold}
...
Working directory: ${projectPath || "(attached project)"}
Name real paths from the excerpts. Do not invent files.`;
  }

  const extraParts = [opts.extraContext || ""];
  if (projectContext) {
    extraParts.push(
      "A real local project is attached. Name real files from the excerpts when relevant."
    );
  }
  const extraContext = extraParts.filter(Boolean).join("\n");
  ...
  const improved = improvePromptLocal({
    ...
    strengthId,
    extraContext,
    // Keep template shape; project excerpts still allow medium/strong shells around them
  ...
  return {
    ...
    meta: {
      ...
      mode: projectContext
        ? templateId
          ? "project+direction+template+local"
          : "project+direction+local"
        : templateId
          ? "direction+template+local"
          : "direction+local",
      direction: direction.id,
      directionLabel: direction.label,
      templateId,
